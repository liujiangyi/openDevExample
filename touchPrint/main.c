...
void Load_Drow_Dialog ( void )
{
    ...
    LCD_ShowString ( lcddev.width - 40, 0, 200, 24, 24, "RST" ); //显示清屏区域
...
void ctp_test ( void )
{
	...
	for ( t = 0; t < CT_MAX_TOUCH; t++ )
	{
		...
		if ( tp_dev.x[t] < lcddev.width && tp_dev.y[t] < lcddev.height )
		{
		    ...
		    lastpos[t][0] = tp_dev.x[t];
		    lastpos[t][1] = tp_dev.y[t];
		    if ( tp_dev.x[t] > ( lcddev.width - 40 ) && tp_dev.y[t] < 16 )
		    {
			Load_Drow_Dialog();//清除
		    }
		}
...
int main ( void )
{
    ...
    uart_init ( 2000000);	 	//串口初始化为2000000
